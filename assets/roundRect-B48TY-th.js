(() => {
	...
	function roundRect(x, y, w, h, radii) {
		...
		const negativeCorner = corners.find(({ x: x$1, y: y$1 }) => x$1 < 0 || y$1 < 0);
		negativeCorner?.x < 0 ? negativeCorner.x : negativeCorner?.y;
		if (corners.some(({ x: x$1, y: y$1 }) => !Number.isFinite(x$1) || !Number.isFinite(y$1))) return;
		...
		function toDOMPointInit(value) {
			const { x: x$1, y: y$1, z, w: w$1 } = value;
			return {
				x: x$1,
				y: y$1,
				z,
		...
		function fixOverlappingCorners(corners$1) {
			const [upperLeft$1, upperRight$1, lowerRight$1, lowerLeft$1] = corners$1;
			const factors = [
				Math.abs(w) / (upperLeft$1.x + upperRight$1.x),
				Math.abs(h) / (upperRight$1.y + lowerRight$1.y),
				Math.abs(w) / (lowerRight$1.x + lowerLeft$1.x),
				Math.abs(h) / (upperLeft$1.y + lowerLeft$1.y)
			];
			const minFactor = Math.min(...factors);
			if (minFactor <= 1) for (const radii$1 of corners$1) {
				radii$1.x *= minFactor;
				radii$1.y *= minFactor;
			}
		}
