import { r as __export } from "./rolldown-runtime-p26lAE5v.js";
import { T as decodeNamedCharacterReference } from "./react-markdown-14W_w1u6.js";
import { E as log$1, Zt as dedent, d as decodeEntities } from "./mermaid-b5860b54-_wCtya72.js";
var emptyOptions = {};
...
function splice(list$1, start, remove, items) {
	const end = list$1.length;
	let chunkStart = 0;
	let parameters;
	...
	if (items.length < 1e4) {
		...
		list$1.splice(...parameters);
	} else {
		if (remove) list$1.splice(start, remove);
...
function push(list$1, items) {
	if (list$1.length > 0) {
		splice(list$1, list$1.length, 0, items);
		return list$1;
	}
	return items;
}
...
function constructs(existing, list$1) {
	let index$1 = -1;
	const before = [];
	while (++index$1 < list$1.length) (list$1[index$1].add === "after" ? existing : before).push(list$1[index$1]);
...
function resolveAllAttention(events, context) {
	...
	while (++index$1 < events.length) if (events[index$1][0] === "enter" && events[index$1][1].type === "attentionSequence" && events[index$1][1]._close) {
		...
		while (open--) if (events[open][0] === "exit" && events[open][1].type === "attentionSequence" && events[open][1]._open && context.sliceSerialize(events[open][1]).charCodeAt(0) === context.sliceSerialize(events[index$1][1]).charCodeAt(0)) {
			...
			const end = Object.assign({}, events[index$1][1].start);
			movePoint(start, -use);
			movePoint(end, use);
			openingSequence = {
			...
			events[index$1][1].start = Object.assign({}, closingSequence.end);
			nextEvents = [];
...
function tokenizeAttention(effects, ok) {
	...
	function inside(code) {
		...
		const open = !after || after === 2 && before || attentionMarkers$1.includes(code);
...
function tokenizeCodeFenced(effects, ok, nok) {
	...
	function tokenizeCloseStart(effects$1, ok$1, nok$1) {
		let size = 0;
		return startBefore;
		...
		function start$1(code) {
			...
			return markdownSpace(code) ? factorySpace(effects$1, beforeSequenceClose, "linePrefix", self.parser.constructs.disable.null.includes("codeIndented") ? void 0 : 4)(code) : beforeSequenceClose(code);
		}
		...
		function sequenceClose(code) {
			...
			if (size >= sizeOpen) {
				...
				return markdownSpace(code) ? factorySpace(effects$1, sequenceCloseAfter, "whitespace")(code) : sequenceCloseAfter(code);
			}
			return nok$1(code);
		}
...
function tokenizeFurtherStart(effects, ok, nok) {
	...
	function afterPrefix(code) {
		const tail = self.events[self.events.length - 1];
		return tail && tail[1].type === "linePrefix" && tail[2].sliceSerialize(tail[1], true).length >= 4 ? ok(code) : markdownLineEnding(code) ? furtherStart$1(code) : nok(code);
	}
}
const codeText = {
	name: "codeText",
	...
	while (++index$1 < events.length) {
		...
		if (event[0] === "enter") {
			...
			if (lineIndex) {
				...
				splice(events, lineIndex, index$1 - lineIndex + 1, parameters);
			}
		}
	}
	return !more;
...
function subcontent(events, eventIndex) {
	...
	while (index$1--) {
		...
		jumps.unshift([start$1, start$1 + slice.length - 1]);
		splice(events, start$1, 2, slice);
	}
	index$1 = -1;
...
function tokenizeHtmlFlow(effects, ok, nok) {
	...
	function cdataOpenInside(code) {
		if (code === "CDATA[".charCodeAt(index$1++)) {
			effects.consume(code);
			if (index$1 === 6) return self.interrupt ? ok : continuation;
			return cdataOpenInside;
		}
		return nok(code);
	}
...
function resolveToSetextUnderline(events, context) {
	...
	while (index$1--) if (events[index$1][0] === "enter") {
		...
		if (events[index$1][1].type === "paragraph") text$2 = index$1;
	} else {
	...
	const heading = {
		type: "setextHeading",
		start: Object.assign({}, events[text$2][1].start),
		end: Object.assign({}, events[events.length - 1][1].end)
	};
	events[text$2][1].type = "setextHeadingText";
	if (definition$1) {
		events.splice(text$2, 0, [
			"enter",
			heading,
			context
		]);
		...
		events[content$2][1].end = Object.assign({}, events[definition$1][1].end);
	} else events[content$2][1] = heading;
	events.push([
		"exit",
		heading,
		context
...
function tokenizeSetextUnderline(effects, ok, nok) {
	...
	function start(code) {
		...
		while (index$1--) if (self.events[index$1][1].type !== "lineEnding" && self.events[index$1][1].type !== "linePrefix" && self.events[index$1][1].type !== "content") {
			paragraph = self.events[index$1][1].type === "paragraph";
			break;
		}
...
function initializeFactory(field) {
	...
	function initializeText(effects) {
		...
		const text$2 = effects.attempt(constructs$1, start, notText);
		return start;
		function start(code) {
			return atBreak(code) ? text$2(code) : notText(code);
		}
		...
		function atBreak(code) {
			...
			const list$1 = constructs$1[code];
			let index$1 = -1;
			if (list$1) while (++index$1 < list$1.length) {
				const item = list$1[index$1];
		...
		function hook(constructs$1, returnState, bogusState) {
			...
			function handleMapOfConstructs(map) {
				...
				function start(code) {
					...
					const all$1 = code !== null && map.null;
					return handleListOfConstructs([...Array.isArray(def) ? def : def ? [def] : [], ...Array.isArray(all$1) ? all$1 : all$1 ? [all$1] : []])(code);
				}
			}
			function handleListOfConstructs(list$1) {
				listOfConstructs = list$1;
				constructIndex = 0;
				if (list$1.length === 0) return bogusState;
				return handleConstruct(list$1[constructIndex]);
			}
...
function compiler(options) {
	const config = {
		...
		enter: {
			...
			listOrdered: opener(list$1, onenterlistordered),
			listUnordered: opener(list$1),
			...
			thematicBreak: opener(thematicBreak$1)
		},
		exit: {
			atxHeading: closer(),
	...
	function compile(events) {
		...
		while (++index$1 < events.length) {
			...
			if (own.call(handler, events[index$1][1].type)) handler[events[index$1][1].type].call(Object.assign({ sliceSerialize: events[index$1][2].sliceSerialize }, context), events[index$1][1]);
		}
	...
	function prepareList(events, start, length) {
		...
		while (++index$1 <= length) {
			...
			if (event[1].type === "listUnordered" || event[1].type === "listOrdered" || event[1].type === "blockQuote") {
				...
				if (event[0] === "enter") {
					if (listItem$1 && !atMarker && !containerBalance && !firstBlankLineIndex) firstBlankLineIndex = index$1;
					atMarker = void 0;
				}
			...
			if (!containerBalance && event[0] === "enter" && event[1].type === "listItemPrefix" || containerBalance === -1 && event[0] === "exit" && (event[1].type === "listUnordered" || event[1].type === "listOrdered")) {
				if (listItem$1) {
					...
					if (firstBlankLineIndex && (!lineIndex || firstBlankLineIndex < lineIndex)) listItem$1._spread = true;
					listItem$1.end = Object.assign({}, lineIndex ? events[lineIndex][1].start : event[1].end);
	...
	function enter(node$1, token, errorHandler) {
		...
		node$1.position = { start: point(token.start) };
		return node$1;
	}
	function closer(and) {
		return close;
	...
	function onexitsetextheadinglinesequence(token) {
		const node$1 = this.stack[this.stack.length - 1];
		node$1.depth = this.sliceSerialize(token).charCodeAt(0) === 61 ? 1 : 2;
	}
	...
	function onenterdata(token) {
		const node$1 = this.stack[this.stack.length - 1];
		let tail = node$1.children[node$1.children.length - 1];
		if (!tail || tail.type !== "text") {
			tail = text$2();
			tail.position = { start: point(token.start) };
			node$1.children.push(tail);
		}
...
function extension(combined, extension$1) {
	let key;
	for (key in extension$1) if (own.call(extension$1, key)) {
...
function markdownToLines(markdown) {
	...
	const lines = [[]];
	let currentLine = 0;
	function processNode(node$1, parentType = "normal") {
		if (node$1.type === "text") node$1.value.split("\n").forEach((textLine, index$1) => {
			if (index$1 !== 0) {
				currentLine++;
				lines.push([]);
			}
		...
		else if (node$1.type === "strong" || node$1.type === "emphasis") node$1.children.forEach((contentNode) => {
			processNode(contentNode, node$1.type);
		});
	}
...
function markdownToHTML(markdown) {
	...
	function output(node$1) {
		if (node$1.type === "text") return node$1.value.replace(/\n/g, "<br/>");
		else if (node$1.type === "strong") return `<strong>${node$1.children.map(output).join("")}</strong>`;
		else if (node$1.type === "emphasis") return `<em>${node$1.children.map(output).join("")}</em>`;
		else if (node$1.type === "paragraph") return `<p>${node$1.children.map(output).join("")}</p>`;
		return `Unsupported markdown: ${node$1.type}`;
	}
	return children.map(output).join("");
}
function splitTextToChars(text$2) {
	if (Intl.Segmenter) return [...new Intl.Segmenter().segment(text$2)].map((s) => s.segment);
	return [...text$2];
}
...
function splitLineToFitWidth(line, checkFit) {
	if (line.some(({ content: content$2 }) => content$2.includes("\n"))) throw new Error("splitLineToFitWidth does not support newlines in the line");
...
function addHtmlSpan(element, node$1, width, classes, addBackground = false) {
	const fo = element.append("foreignObject");
	const div = fo.append("xhtml:div");
	const label = node$1.label;
	const labelClass = node$1.isNode ? "nodeLabel" : "edgeLabel";
	div.html(`
    <span class="${labelClass} ${classes}" ` + (node$1.labelStyle ? "style=\"" + node$1.labelStyle + "\"" : "") + ">" + label + "</span>");
	applyStyle(div, node$1.labelStyle);
...
function computeDimensionOfText(parentNode, lineHeight, text$2) {
	var _a;
	const testElement = parentNode.append("text");
	const testSpan = createTspan(testElement, 1, lineHeight);
...
function updateTextContentAndStyles(tspan, wrappedLine) {
	tspan.text("");
	wrappedLine.forEach((word, index$1) => {
		const innerTspan = tspan.append("tspan").attr("font-style", word.type === "emphasis" ? "italic" : "normal").attr("class", "text-inner-tspan").attr("font-weight", word.type === "strong" ? "bold" : "normal");
		if (index$1 === 0) innerTspan.text(word.content);
		else innerTspan.text(" " + word.content);
	});
}
var createText = (el, text$2 = "", { style = "", isTitle = false, classes = "", useHtmlLabels = true, isNode = true, width = 200, addSvgBackground = false } = {}) => {
	log$1.info("createText", text$2, style, isTitle, classes, useHtmlLabels, isNode, addSvgBackground);
	if (useHtmlLabels) return addHtmlSpan(el, {
		isNode,
		label: decodeEntities(markdownToHTML(text$2)).replace(/fa[blrs]?:fa-[\w-]+/g, (s) => `<i class='${s.replace(":", " ")}'></i>`),
		labelStyle: style.replace("fill:", "color:")
	}, width, classes, addSvgBackground);
	else return createFormattedText(width, el, markdownToLines(text$2), addSvgBackground);
};
