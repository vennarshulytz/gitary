import { _i as select_default, bi as array, vi as Selection, yi as root } from "./app-DQCpdmdF.js";
import { B as utils, Dt as isFunction_default, E as log$1, It as linear_default, c as common$1, j as renderKatex, jt as rgba_default, m as evaluate, st as isPlainObject_default, v as getConfig, w as interpolateToCurve, x as getStylesFromArray, z as setupGraphViewbox$1 } from "./mermaid-b5860b54-_wCtya72.js";
import { t as channel_default } from "./channel-DQOg61k1.js";
import { t as Graph } from "./graphlib-CtKmUqMg.js";
import { t as render } from "./index-3862675e-3PG0tCW9.js";
...
function applyTransition(selection, g) {
	...
	if (isPlainObject_default(graph)) {
		...
		if (isFunction_default(transition)) return transition(selection);
	}
	return selection;
}
function addHtmlLabel(root$1, node) {
	var fo = root$1.append("foreignObject").attr("width", "100000");
...
var addVertices = async function(vert, g, svgId, root$1, doc, diagObj) {
	const svg = root$1.select(`[id="${svgId}"]`);
...
var draw = async function(text, id, _version, diagObj) {
	...
	const root$1 = securityLevel === "sandbox" ? select_default(sandboxElement.nodes()[0].contentDocument.body) : select_default("body");
	...
	await addVertices(vert, g, id, root$1, doc, diagObj);
	await addEdges(edges, g);
	const svg = root$1.select(`[id="${id}"]`);
	await render(root$1.select("#" + id + " g"), g, [
		"point",
		"circle",
		"cross"
	], "flowchart", id);
...
var fade = (color, opacity) => {
	...
	return rgba_default(channel(color, "r"), channel(color, "g"), channel(color, "b"), opacity);
};
