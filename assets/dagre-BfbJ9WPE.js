import { Ai as now_default, Di as isObjectLike_default, Ei as isSymbol_default, Ni as isObject_default, Oi as _baseGetTag_default, Ti as toNumber_default } from "./app-DQCpdmdF.js";
import { $ as keysIn_default, Et as eq_default, G as _baseKeys_default, J as _baseRest_default, K as merge_default, Q as identity_default, St as _baseAssignValue_default, U as _getTag_default, X as constant_default, Y as _setToString_default, Z as _overRest_default, ft as isArray_default, ot as _baseUnary_default, q as _isIterateeCall_default, rt as _assignValue_default, tt as _isIndex_default, ut as isArrayLike_default, xt as _baseFor_default } from "./mermaid-b5860b54-_wCtya72.js";
import { S as keys_default, _ as _castPath_default, a as has_default, b as _arrayMap_default, c as _castFunction_default, d as _baseIteratee_default, f as _baseProperty_default, g as _toKey_default, h as _baseGet_default, i as values_default, l as _baseEach_default, m as _baseFlatten_default, n as reduce_default, o as filter_default, p as hasIn_default, r as isUndefined_default, s as forEach_default, t as Graph, u as _baseForOwn_default, v as toString_default, x as _baseClone_default, y as _baseFindIndex_default } from "./graphlib-CtKmUqMg.js";
...
var defaults_default = _baseRest_default(function(object, sources) {
	...
	while (++index < length) {
		...
		while (++propsIndex < propsLength) {
			...
			if (value === void 0 || eq_default(value, objectProto[key]) && !hasOwnProperty.call(object, key)) object[key] = source[key];
		}
	}
	return object;
});
...
function dfsFAS(g) {
	...
	function dfs$2(v) {
		...
		forEach_default(g.outEdges(v), function(e) {
			if (has_default(stack, e.w)) fas.push(e);
			else dfs$2(e.w);
		});
		delete stack[v];
	}
...
function reverseY(g) {
	...
	forEach_default(g.edges(), function(e) {
		...
		if (has_default(edge, "y")) reverseYOne(edge);
	});
}
...
function swapXY(g) {
	...
	forEach_default(g.edges(), function(e) {
		...
		if (has_default(edge, "x")) swapXYOne(edge);
	});
}
...
function longestPath(g) {
	...
	function dfs$2(v) {
		var label = g.node(v);
		if (has_default(visited, v)) return label.rank;
		visited[v] = true;
...
function topsort(g) {
	...
	var results = [];
	function visit(node) {
		if (has_default(stack, node)) throw new CycleException();
...
function dfsAssignLowLim(tree, visited, nextLim, v, parent) {
	...
	forEach_default(tree.neighbors(v), function(w) {
		if (!has_default(visited, w)) nextLim = dfsAssignLowLim(tree, visited, nextLim, w, v);
	});
	label.low = low;
	label.lim = nextLim++;
...
function buildLayerGraph(g, rank$1, relationship) {
	...
	forEach_default(g.nodes(), function(v) {
		...
		if (node.rank === rank$1 || node.minRank <= rank$1 && rank$1 <= node.maxRank) {
			...
			if (has_default(node, "minRank")) result.setNode(v, {
...
function sortSubgraph(g, v, cg, biasRight) {
	...
	forEach_default(barycenters, function(entry) {
		if (g.children(entry.v).length) {
			...
			if (has_default(subgraphResult, "barycenter")) mergeBarycenters(entry, subgraphResult);
		}
	});
...
function sep(nodeSep, edgeSep, reverseSep) {
	return function(g, v, w) {
		...
		var delta;
		sum += vLabel.width / 2;
		if (has_default(vLabel, "labelpos")) switch (vLabel.labelpos.toLowerCase()) {
			case "l":
		...
		sum += wLabel.width / 2;
		if (has_default(wLabel, "labelpos")) switch (wLabel.labelpos.toLowerCase()) {
			case "l":
...
function translateGraph(g) {
	...
	forEach_default(g.edges(), function(e) {
		var edge = g.edge(e);
		if (has_default(edge, "x")) getExtremes(edge);
	});
	minX -= marginX;
	minY -= marginY;
	forEach_default(g.nodes(), function(v) {
	...
	forEach_default(g.edges(), function(e) {
		...
		if (has_default(edge, "x")) edge.x -= minX;
		if (has_default(edge, "y")) edge.y -= minY;
	});
...
export { map_default as a, pick_default as i, uniqueId_default as n, defaults_default as o, range_default as r, layout as t };
