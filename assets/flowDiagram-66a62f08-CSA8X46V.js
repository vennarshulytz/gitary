...
import { a as applyStyle, c as isSubgraph, i as applyClass, l as selectAll_default, n as flowStyles, o as applyTransition, r as addHtmlLabel, s as edgeToId, t as flowRendererV2 } from "./styles-c10674c1-D28zeVR0.js";
import "./monaco-DGZQwLgt.js";
import "./react-markdown-CC4NHZ99.js";
import { _i as select_default, ri as require_dist } from "./app-BiKdazfq.js";
import { E as log$1, It as linear_default, Pt as require_purify, Xt as require_dayjs_min, c as common$1, j as renderKatex, m as evaluate, v as getConfig, w as interpolateToCurve, x as getStylesFromArray, z as setupGraphViewbox$1 } from "./mermaid-b5860b54-BdwE7Q2A.js";
import "./path-i3PwAG1d.js";
import "./array-BUZ-AMCX.js";
import { t as line_default } from "./line-S-HVn8G8.js";
import "./channel-B3hplsj_.js";
import { a as has_default, s as forEach_default, t as Graph } from "./graphlib-CKQum7-R.js";
import "./clone-DvD8f-VH.js";
import "./createText-2e5e7dd3-D5_s_NBi.js";
import "./edges-e0da2a9e-CyvX9dQm.js";
import { i as pick_default, n as uniqueId_default, o as defaults_default, r as range_default, t as layout } from "./dagre-BDKgcGXU.js";
...
function addLabel(root, node, location$1) {
...
var createEdgeLabels = function(selection, g) {
	...
	svgEdgeLabels.each(function(e) {
		...
		if (!has_default(edge, "width")) edge.width = bbox.width;
		if (!has_default(edge, "height")) edge.height = bbox.height;
	});
	var exitSelection;
...
var createEdgePaths = function(selection, g, arrows$1) {
	...
	svgPaths.selectAll("path.path").each(function(e) {
		...
		applyTransition(domEdge, g).attr("d", function(e$1) {
			return calcPoints(g, e$1);
		});
	...
	svgPaths.selectAll("defs").each(function(e) {
		var edge = g.edge(e);
		var arrowhead = arrows$1[edge.arrowhead];
...
function enter(svgPaths, g) {
	...
	svgPathsEnter.append("path").attr("class", "path").attr("d", function(e) {
		...
		return createLine(edge, range_default(edge.points.length).map(function() {
...
var createNodes = function(selection, g, shapes$1) {
	...
	svgNodes.each(function(v) {
		...
		var shape = shapes$1[node.shape];
		var bbox = pick_default(labelDom.node().getBBox(), "width", "height");
		node.elem = this;
		if (node.id) thisGroup.attr("id", node.id);
		if (node.labelId) labelGroup.attr("id", node.labelId);
		if (has_default(node, "width")) bbox.width = node.width;
		if (has_default(node, "height")) bbox.height = node.height;
...
function render() {
	var fn = function(svg$1, g) {
		preProcessGraph(g);
		var outputGroup = createOrSelectGroup(svg$1, "output");
...
function preProcessGraph(g) {
	g.nodes().forEach(function(v) {
		var node = g.node(v);
		if (!has_default(node, "label") && !g.children(v).length) node.label = v;
		if (has_default(node, "paddingX")) defaults_default(node, {
			paddingLeft: node.paddingX,
			paddingRight: node.paddingX
		});
		if (has_default(node, "paddingY")) defaults_default(node, {
			paddingTop: node.paddingY,
			paddingBottom: node.paddingY
		});
		if (has_default(node, "padding")) defaults_default(node, {
		...
		if (has_default(node, "width")) node._prevWidth = node.width;
		if (has_default(node, "height")) node._prevHeight = node.height;
	});
	g.edges().forEach(function(e) {
		var edge = g.edge(e);
		if (!has_default(edge, "label")) edge.label = "";
...
function postProcessGraph(g) {
	forEach_default(g.nodes(), function(v) {
		var node = g.node(v);
		if (has_default(node, "_prevWidth")) node.width = node._prevWidth;
		else delete node.width;
		if (has_default(node, "_prevHeight")) node.height = node._prevHeight;
...
var addVertices = async function(vert, g, svgId, root, _doc, diagObj) {
	const svg$1 = !root ? select_default(`[id="${svgId}"]`) : root.select(`[id="${svgId}"]`);
	...
	for (const id of keys) {
		...
		if (evaluate(getConfig().flowchart.htmlLabels)) {
			vertexNode = addHtmlLabel(svg$1, { label: await renderKatex(vertexText.replace(/fa[blrs]?:fa-[\w-]+/g, (s) => `<i class='${s.replace(":", " ")}'></i>`), getConfig()) }).node();
	...
	for (const edge of edges) {
		...
		if (edge.text === void 0) {
			...
			if (evaluate(getConfig().flowchart.htmlLabels)) {
				...
				edgeData.label = `<span id="L-${linkId}" class="edgeLabel L-${linkNameStart}' L-${linkNameEnd}" style="${edgeData.labelStyle}">${await renderKatex(edge.text.replace(/fa[blrs]?:fa-[\w-]+/g, (s) => `<i class='${s.replace(":", " ")}'></i>`), getConfig())}</span>`;
			} else {
...
var draw = async function(text, id, _version, diagObj) {
	...
	render$1.arrows().none = function normal$1(parent, id2, edge, type) {
	...
	render$1.arrows().normal = function normal$1(parent, id2) {
	...
	if (!conf2.htmlLabels) {
		...
		for (const label of labels) {
			...
			const rect$1 = doc.createElementNS("http://www.w3.org/2000/svg", "rect");
			rect$1.setAttribute("rx", 0);
			rect$1.setAttribute("ry", 0);
			rect$1.setAttribute("width", dim.width);
			rect$1.setAttribute("height", dim.height);
			label.insertBefore(rect$1, label.firstChild);
		}
	}
	setupGraphViewbox$1(g, svg$1, conf2.diagramPadding, conf2.useMaxWidth);
